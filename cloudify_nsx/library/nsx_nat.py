...
def nat_service(client_session, esg_id, enabled):
    ...
    current_nat_config = client_session.read('edgeNat', uri_parameters={'edgeId': esg_id})['body']
    ...
    if not change_needed:
        ...
        result = client_session.update('edgeNat', uri_parameters={'edgeId': esg_id},
...
def add_nat_rule(client_session, esg_id, action, originalAddress,
    translatedAddress, vnic=None, ruleTag=None, loggingEnabled=False,
    enabled=True, description='', protocol='any', translatedPort='any',
    originalPort='any'):

    nat_spec = client_session.extract_resource_body_example('edgeNatRules', 'create')
    ...
    result = client_session.create('edgeNatRules', uri_parameters={'edgeId': esg_id}, request_body_dict=nat_spec)
...
def delete_nat_rule(client_session, esg_id, resource_id):
    result = client_session.delete('edgeNatRule', uri_parameters={'edgeId': esg_id, 'ruleID': resource_id})
